...
for cnt = 1:length(Potential_y0)
    if (Accumulatortemp(Potential_y0(cnt),Potential_x0(cnt)) >= 0)
        if isempty(x0detect)           
            y0detect = Potential_y0(cnt);
            x0detect = Potential_x0(cnt);
            point_count = 1;
        else
            point = find(sqrt((y0detect-Potential_y0(cnt)).^2+(x0detect-Potential_x0(cnt)).^2)<r);
            if isempty(point)
                y0detect = [y0detect;Potential_y0(cnt)];
                x0detect = [x0detect;Potential_x0(cnt)];
                point_count = [point_count 1]
            else
                y0detect(point) = (point_count*y0detect(point)+Potential_y0(cnt))/(point_count(point)+1);
                x0detect(point) = (point_count*x0detect(point)+Potential_x0(cnt))/(point_count(point)+1);
