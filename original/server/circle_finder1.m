...
img = imread('coinswref4.jpg');
hsvimg = rgb2hsv(img);
quarter_rad_x = (2000-1575)/2;
quarter_rad_y = (655-230)/2;
% ref_col = [.6694 .94 .88]';
% for j = 25:50,
...
%         current_img_hsv = rgb2hsv(current_img_color);
        current_img_hue = himg.*current_img_bw;
        current_img_hue = current_img_hue(current_img_hue>0);
        penny_threshold = 0.1;
        ratio = (length(find((current_img_hue>penny_threshold) .* (current_img_hue<lower_thresh))) + length(find((current_img_hue>upper_thresh).*(current_img_hue<(1-penny_threshold)))))/...
            (length(find((current_img_hue<penny_threshold))) + length(find(current_img_hue>(1-penny_threshold))))
         ratios = [ratios ratio];
        subplot(122)
...
for j = 1:length(xlength_arr)
    ...
    if ((ratios(j)<.2) && ((min(penny_rad_dists)/penny_rad_dists(2))>.25))
        numpennys = numpennys+1;
    else
